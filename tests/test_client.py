...
def make_client(**kwargs) -> Client:
    defaults = {"in_memory": True, "interactive": False, "enable_socket": False, "transport": "http"}
...
def test_invoke_raw_calls_any_method_by_name(monkeypatch):
    async def scenario():
        ...
        assert isinstance(result, types.RawObject) and result.get("answer") == 42
...
def test_rpc_errors_are_mapped_to_typed_exceptions(monkeypatch):
    async def scenario():
        ...
        rubika.responses["getChats"] = {
            "status": "ERROR_GENERIC",
            "status_det": "TOO_REQUESTS",
            "client_show_message": {"link": {"alert_data": {"message": "لطفا ۳۰ ثانیه دیگر تلاش کنید"}}},
        }
...
def test_login_flow_uses_tmp_session_then_stores_unwrapped_auth(monkeypatch):
    async def scenario():
        ...
        rubika.responses["signIn"] = {
            "status": "OK",
            "auth": rsa_wrap(public_key, new_auth),
            "user_guid": FAKE_USER_GUID,
            "user": {"user_guid": FAKE_USER_GUID, "first_name": "Sample"},
        }
        ...
        assert (
            set(send_payload) == {"api_version", "tmp_session", "data_enc"} and send_payload["tmp_session"] == sign_payload["tmp_session"]
        )
...
def test_send_message_builds_metadata_rnd_and_returns_sent_message(monkeypatch):
    async def scenario():
        ...
        sent = await client.send_message(
            "u0EXAMPLE00000000000000000000002", "**bold** text", parse_mode="markdown", reply_to_message_id="10"
        )
...
def test_message_helpers_map_to_the_right_rpcs(monkeypatch):
    async def scenario():
        client, rubika, _ = await started(monkeypatch)
        rubika.responses.update(
            {
                "editMessage": {},
                "deleteMessages": {},
                "forwardMessages": {},
                "setPinMessage": {},
                "getMessages": {"messages": [{"message_id": "5", "text": "x"}, {"message_id": "4"}], "has_continue": True},
                "getMessagesByID": {"messages": [{"message_id": "7", "text": "seven"}]},
        ...
        assert rubika.inputs("editMessage")[0] == {
            "object_guid": "g1",
            "message_id": "1",
            "text": "new text",
            "metadata": {"meta_data_parts": [{"type": "Italic", "from_index": 4, "length": 4}]},
        }
        assert rubika.inputs("deleteMessages") == [
            {"object_guid": "g1", "message_ids": ["1"], "type": "Global"},
            {"object_guid": "g1", "message_ids": ["1", "2"], "type": "Local"},
        ]
        ...
        assert (
            poll["type"] == "Quiz" and poll["correct_option_index"] == 1 and poll["options"] == ["a", "b"] and poll["is_anonymous"] is True
        )
...
def test_chats_and_users_mixins(monkeypatch):
    async def scenario():
        ...
        rubika.responses.update(
            {
                "getChats": {"chats": [{"object_guid": "g1", "last_message": {"message_id": "1"}}], "has_continue": False},
                "setActionChat": {},
                "setBlockUser": {},
                "getContactsUpdates": {"new_state": 99, "users": []},
                "updateProfile": {},
            }
        )
...
def test_groups_channels_and_join_links(monkeypatch):
    async def scenario():
        ...
        assert rubika.inputs("editGroupInfo")[0] == {
            "group_guid": "g1",
            "title": "T",
            "slow_mode": 10,
            "updated_parameters": ["title", "slow_mode"],
        }
        ...
        assert rubika.inputs("createJoinLink")[0] == {
            "object_guid": "g1",
            "title": "invite",
...
def test_send_media_requests_slot_uploads_then_sends_file_inline(monkeypatch, tmp_path):
    async def scenario():
        client, rubika, _ = await started(monkeypatch)
        client._upload = cast(Any, StubUpload())
        ...
        upload = cast(Any, client._upload).calls[0]
        ...
        assert (
            message["file_inline"]["file_id"] == "87998036915657"
            and message["file_inline"]["dc_id"] == "491"
            and message["file_inline"]["access_hash_rec"] == "rec-hash"
        )
...
def test_download_file_uses_the_storage_of_the_dc(monkeypatch, tmp_path):
    async def scenario():
        client, rubika, _ = await started(monkeypatch)
        client._download = cast(Any, StubDownload())
        message = types.Message(
            client=client,
            message_id="1",
            file_inline=types.FileInline._parse(
                client, {"file_id": "f1", "dc_id": "491", "access_hash_rec": "r", "file_name": "a.bin", "size": "3"}
            ),
        )
        target = await client.download_file(message, tmp_path)
        assert isinstance(target, Path) and target == tmp_path / "a.bin" and target.read_bytes() == b"bytes"
        call = cast(Any, client._download).calls[0]
...
def test_rubino_post_goes_to_the_suggested_rubino_url(monkeypatch):
    async def scenario():
        client, rubika, services = await started(monkeypatch)
        services.responses["getProfilePosts"] = {
            "status": "OK",
            "data": {"posts": [{"id": "p1", "profile_id": "pr1", "post_type": "Picture", "full_file_url": "https://cdn/x.jpg"}]},
        }
