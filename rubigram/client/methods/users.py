...
class Users(BaseClient):
    ...
    async def get_user_info(self, user_guid: Any) -> UserInfo:
        """``getUserInfo``. [HTTP]"""
    ...
    async def get_abs_objects(self, object_guids: Sequence[Any]) -> AbsObjects:
    ...
    async def get_avatars(self, object_guid: Any = None, *, peer: Any = None) -> ChatAvatars:
    ...
    async def update_username(self, username: str) -> UpdatedProfile:
        """``updateUsername``. [HTTP]"""
    ...
    async def set_block_user(self, user_guid: Any, action: Any = "Block") -> RawObject:
    ...
    async def block_user(self, object_guid: Any = None, *, peer: Any = None) -> RawObject:
        """Block a user (``setBlockUser`` / ``Block``). [HTTP]"""
        return await self.set_block_user(peer if peer is not None else object_guid, "Block")

    async def unblock_user(self, object_guid: Any = None, *, peer: Any = None) -> RawObject:
    ...
    async def get_blocked_users(self, start_id: Optional[str] = None) -> BlockedUsers:
        """``getBlockedUsers``. [HTTP]"""
        return await self.invoke(GetBlockedUsers(start_id=start_id))

    async def get_contacts(self, start_id: Optional[str] = None) -> Contacts:
        """A page of contacts (``getContacts``); pass ``next_start_id`` to continue. [HTTP]"""
        return await self.invoke(GetContacts(start_id=start_id))

    async def get_contacts_updates(self, state: Optional[int] = None) -> ContactsUpdates:
    ...
    async def get_contacts_last_online(self, user_guids: Sequence[Any]) -> ContactsLastOnline:
    ...
    async def add_contact(self, phone: str, first_name: str, last_name: str = "") -> UpdatedProfile:
    ...
    async def delete_contact(self, user_guid: Any) -> Empty:
        """``deleteContact``. [HTTP]"""
        return await self.invoke(DeleteContact(user_guid=self._resolve_user_guid(user_guid)))

    async def reset_contacts(self) -> Empty:
        """``resetContacts``. [HTTP]"""
        return await self.invoke(ResetContacts())

    async def get_profile_link_items(self, object_guid: Any = None, *, peer: Any = None) -> ProfileLinkItems:
    ...
    async def get_common_groups(self, user_guid: Any) -> CommonGroups:
    ...
    async def set_ask_spam_action(self, object_guid: Any, action: str) -> RawObject:
    ...
    async def search_global_objects(self, search_text: str, filter_types: Sequence[Any] = ()) -> SearchGlobalObjectsResult:
