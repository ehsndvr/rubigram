...
class BotApi(BaseClient):
    async def _bot_call(self, method: str, payload: Optional[Dict[str, Any]] = None) -> Any:
        ...
        try:
            ...
            if method == "sendMessage" and payload and str(payload.get("chat_id", "")).startswith("u0"):
                raise InvalidInput(
                    ...
                    exc.raw,
                    method=method,
                    client_show_message="Bot sendMessage expects chat_id (from get_updates()/webhooks/get_chat()), not a user_guid",
                ) from exc
            raise
    ...
    async def _bot_send_poll(self, chat_id: str, question: str, options: list[str]) -> SentMessage:
    ...
    async def _bot_forward_message(
        ...
        return SentMessage._parse(
            self,
            await self._bot_call(
                "forwardMessage",
                {
    ...
    async def _bot_edit_message_text(self, chat_id: str, message_id: str, text: str) -> SentMessage:
        return SentMessage._parse(
            self, await self._bot_call("editMessageText", {"chat_id": chat_id, "message_id": message_id, "text": text})
        )
    ...
    async def edit_message_text(self, chat_id: Any, message_id: Any, text: str) -> Any:
    ...
    async def edit_message_keypad(self, chat_id: Any, message_id: Any, inline_keypad: Keypad) -> bool:
        """``editMessageKeypad``. [bot]"""
        await self._bot_call(
            "editMessageKeypad",
            {"chat_id": self._resolve_object_guid(chat_id), "message_id": str(message_id), "inline_keypad": _serialize(inline_keypad)},
        )
        return True

    edit_inline_keypad = edit_message_keypad

    async def set_commands(self, bot_commands: Sequence[Union[BotCommand, Dict[str, Any]]]) -> bool:
        """``setCommands``. [bot]"""
    ...
    async def edit_chat_keypad(self, chat_id: Any, *, chat_keypad_type: Any, chat_keypad: Optional[Keypad] = None) -> bool:
        """``editChatKeypad``. [bot]"""
        await self._bot_call(
            "editChatKeypad",
            {
                "chat_id": self._resolve_object_guid(chat_id),
                "chat_keypad_type": str(getattr(chat_keypad_type, "value", chat_keypad_type)),
                "chat_keypad": _serialize(chat_keypad),
            },
        )
        return True
    ...
    async def ban_chat_member(self, chat_id: Any, user_id: str) -> bool:
        """``banChatMember``. [bot]"""
        await self._bot_call("banChatMember", {"chat_id": self._resolve_object_guid(chat_id), "user_id": user_id})
        return True

    async def unban_chat_member(self, chat_id: Any, user_id: str) -> bool:
        """``unbanChatMember``. [bot]"""
        await self._bot_call("unbanChatMember", {"chat_id": self._resolve_object_guid(chat_id), "user_id": user_id})
        return True

    async def parse_webhook_update(self, payload: Dict[str, Any]) -> WebhookUpdate:
